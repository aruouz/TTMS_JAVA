...
public class ScheduleEditUI extends ScheduleAddUI{
	...
	public void initData(Schedule sched) {
        ...
        for(;ssum<listssum.size();ssum++){
        	if(listss.get(0).getID()==listssum.get(ssum).getID()){
        		System.out.println(listss.get(0).getID());
		...
		String y = null,m = null,d = null,h = null,f = null;
		for(int i=0;i<4;i++)
			y+=timeCorrent[i];
		for(int j=5;j<7;j++)
			m+=timeCorrent[j];
		...
		jcbSched_time1.setSelectedIndex(2050-y1);
		jcbSched_time2.setSelectedIndex(12-m1);
		jcbSched_time3.setSelectedIndex(31-d1);
		jcbSched_time4.setSelectedIndex(24-h1);
		jcbSched_time5.setSelectedIndex(59-f1);
	...
	protected void btnSaveClicked(){
		if (jcbStudioName.getSelectedItem() != null && jcbPlayName.getSelectedItem() != null && jcbSched_time.getSelectedItem() != null && txtSched_ticket_price.getText() != null) {
